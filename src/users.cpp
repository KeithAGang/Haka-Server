...
// If User struct was defined in core.hpp or a common header, you wouldn't need
// this here.
struct User {
  int id;
  std::string name;
...
/**
 * @brief Creates and configures a Router specifically for user-related API
 * endpoints.
...
Haka::Router createUserApiRouter() {
  ...
  user_router.Get(
      "/profile", [](const Haka::Request &req, Haka::Response &res) {
  ...
  //     admin_router.Get("/dashboard", ...); // This would be
  //     /users/admin/dashboard if mounted at /users
  // });
