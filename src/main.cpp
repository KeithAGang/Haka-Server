...
Haka::Router createNewEndpoints() {
  ...
  new_router.Get("/", [](const Haka::Request &req, Haka::Response &res) {
    Haka::JsonResponse msg{.title = "Hi", .message = "From new Router"};
...
// This struct is placed here because it's used directly in routes defined in
// main.cpp. For a larger project, common structs would go in a shared header.
...
int main(int argc, char *argv[]) { // Added command-line arguments
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    // JUST some pointer this is doing a heap allocation bad for performance
  ...
  server.Get("/hello", [](const Haka::Request &req, Haka::Response &res) {
    res.HTML("<h1>Hello, Haka!</h1><p>This is an HTML response from the /hello "
             "route.</p>");
  });

  // GET route returning JSON using the default JsonResponse struct at
  // "/status".
  ...
  server.Get("/json", [](const Haka::Request &req, Haka::Response &res) {
    ...
    for (int i = 0; i < 15; ++i) {
      products.push_back({i + 1, fmt::format("Product {}", i + 1),
  ...
  // Call the function from users.cpp to create and configure the user API
  // router instance.
  Haka::Router user_api_router = createUserApiRouter();

  // Mount the user API router under the "/api/users" prefix on the main
  // server's router. Routes defined in user_api_router (like "/list",
  // "/profile") will now be accessible under this prefix (e.g.,
  // "/api/users/list", "/api/users/profile").
  ...
  // --- Serve Static Files ---
  // This will serve files from the "./public" directory under the "/static" URL
  // prefix. Create a 'public' directory in the same location as your executable
  // and put files like index.html, style.css, etc. inside it.
  // For example, "./public/index.html" will be accessible at
  // "http://127.0.0.1:8080/static/index.html".
  server.serveStatic("/static", "./public");

  // --- Start the Server ---
  // This call is blocking and will run the server until interrupted (e.g.,
  // Ctrl+C).
  try {
